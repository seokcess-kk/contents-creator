...
### Phase 4 PR2 ✅ publication 자동 등록 (opt-in, default OFF) — 2026-05-05
...
- **본문 차별화 검증 (Phase 5 PR1 ✅)** — cluster reuse 본문 사후 Jaccard 검증 활성. 단어 3-gram, default 임계값 0.7 (`cluster_body_similarity_threshold`), `cluster_body_similarity_enabled` toggle. 임계값 초과 시 자동 needs_review + compliance_violations 에 `본문_차별화_부족` 추가. **outline negative example 주입은 Phase 5 PR2 후속** (생성 단계 prompt 변경, 운영 데이터 누적 후 재검토)
...
- `2026-05-05`: Phase 5 PR1 — **본문 차별화 검증 (Jaccard)**. cluster_dedupe ON 정책의 1페이지 노출 mitigation. `application/text_similarity.py` 신규 — `jaccard_similarity(a, b, ngram=3)` (단어 n-gram set 기반) + `fetch_content_md(generated_content_id)` (graceful, Supabase 미설정/실패/부재 시 None). `_dispatch_item` 의 cluster reuse + generate/pipeline 분기 직후 `_check_member_body_too_similar` 호출 — 임계값 초과 시 `compliance_violations` 에 `본문_차별화_부족` 추가 + `needs_review` 강제 (compliance_passed=True 라도). settings: `cluster_body_similarity_enabled` (default True), `cluster_body_similarity_threshold` (default 0.7). outline negative example 주입은 Phase 5 PR2 (생성 단계 prompt 변경, 운영 데이터 누적 후 재검토). 테스트 +18 (jaccard 8 + fetch_content_md 5 + _check_member_body_too_similar 4 + _dispatch_item 통합 3).
- `2026-05-05`: Phase 4 PR3 — **검수 큐 임계 알림 + auto_publish UI 토글**. (1) `notifier.send_review_queue_threshold(batch, needs_review_count, threshold)` 신규 + `config/settings.slack_review_queue_threshold` (default 0 = 비활성). `recompute_batch_status` completed 첫 진입 hook 에서 threshold>0 + needs_review>=threshold 시 호출. SPEC §3 Phase 4 의 5종 알림 트리거 중 마지막 항목 활성. (2) `auto_publish_enabled` 전파 — `enqueue_from_csv` / `BatchCreateJsonRequest` / `_extract_csv_input` / `createBatch[File]` 옵션 / `BatchUploadForm` 체크박스 (default unchecked). 운영자가 batch 생성 시 명시 opt-in 가능. **Phase 4 전체 완료** (PR1+PR2+PR3).
- `2026-05-05`: Phase 4 PR2 — **publication 자동 등록 (opt-in)**. `application/auto_publisher.py` 신규 — `auto_publish_ready_items(batch_id)`. `keyword_batches.auto_publish_enabled=True` 인 batch 만 활성, ready_to_publish + target_url 채워진 item 을 `ranking_orchestrator.register_publication` 호출 (멱등 + `_attach_batch_item` 가 publication_id 자동 백필 → /rankings 추적 진입). 자동 트리거: `recompute_batch_status` 가 completed 첫 진입 + auto_publish_enabled=True 면 자동 호출 (graceful — 실패해도 batch status 영향 X). 운영자 트리거: CLI `scripts/run_batch.py --auto-publish <batch_id>` + Web API `POST /batches/{id}/auto-publish` + frontend `autoPublishBatch()` API 함수. 운영 철학 §0 반영 — auto_publish_enabled default False, 운영자 명시 opt-in.
...
- `2026-05-06`: UX Refactor 6 Phase + Polish Pack 4 Phase 완료. **백엔드 무변경** (batch storage / orchestrator / 13 라우터 endpoint 모두 그대로). frontend 라우트 변경 — `/batches/[id]/review` `/batches/[id]/publish` `/results/[slug]` 가 `/queue` 로 영구 redirect (외부 북마크/SEO 보호). CSV 업로드는 `/batches` → `/create?tab=batch` 로 분리. 운영 데이터 의존: 활성 batch 10 × 100 키워드 = 최대 1000 row → **frontend merge 확정** (백엔드 unified_queue use case 신설 0). 라우트 단일 출처: `docs/ROUTES.md`
