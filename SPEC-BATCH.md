...
- `2026-05-04`: Phase 2 PR4 완료 — **Triple link 사후 백필 운영 도구**. `find_pattern_card_by_triple(slug, keyword)` + `find_generated_content_by_triple(job_id, slug, keyword)` storage 신규 (job_id None 시 1차 스킵, slug+keyword fallback). `backfill_unlinked_items(batch_id)` application use case (idempotent). `POST /batches/{id}/backfill-fk` API + `scripts/run_batch.py --backfill-fk` CLI. fire-and-forget 회수 실패 케이스 사후 보강. 자동 cron 미사용. **Phase 2 전체 완료**.
