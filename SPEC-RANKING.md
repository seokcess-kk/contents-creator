...
**입력**: `job_id` (선택), `keyword`, `slug`, `url`, `published_at` (선택)
**처리**:
...
2. `domain.crawler.serp_collector.build_integrated_serp_url(keyword)` 로 SERP URL 생성
3. `BrightDataClient` 으로 SERP HTML fetch
4. `serp_collector._parse_serp_html(html)` 로 결과 파싱 → `list[ParsedSerpItem]`
5. `domain/ranking/tracker.find_position(...)` 호출:
   - 각 item 의 url 을 `urls_match(target_url, item.url)` 로 비교
   - 발견 시 `position = index + 1` (1-based)
   - 미발견 시 `position = None` (= 100위 밖)
6. `RankingSnapshot` 생성 → `storage.insert_snapshot(...)`
7. 선택적으로 SERP HTML 을 Supabase Storage 에 보관 (재현·검증용)
**출력**: `RankingSnapshot` Pydantic 모델

**핵심 설계 — 도메인 격리**:
- `domain/ranking/tracker.py` 는 `domain/crawler/*` 를 **직접 import 하지 않는다**.
- `find_position` 시그니처는 `serp_fetcher: Callable[[str], str]`, `serp_parser: Callable[[str], list[ParsedSerpItem]]` 로 의존성 주입.
- 두 함수의 합성은 **application/ranking_orchestrator.py** 가 책임.
- 부작용: `BLOG_POST_URL_RE` 정규식이 `serp_collector.py` 와 `domain/ranking/url_match.py` 양쪽에 의도적으로 복제됨. **수동 동기화 필요** (lessons.md 명시).
...
```sql
-- 발행 등록 (사용자가 네이버 블로그에 발행한 URL)
create table publications (
  id uuid primary key default uuid_generate_v4(),
  job_id text,                     -- 어느 job 의 결과물인지 (선택, 추적용)
  keyword text not null,           -- 추적 검색어
  slug text not null,              -- output 디렉터리 매칭용
  url text not null unique,        -- 사용자 발행 URL (정규화된 형태)
  published_at timestamptz,        -- 사용자 발행 시점 (선택, 기본 created_at)
