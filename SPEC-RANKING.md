...
[수집]    매일 09:00 KST 모든 활성 publication 순회
   │      → 키워드 SERP 재크롤 (하이브리드: insane #main_pack 우선 + Bright Data 폴백)
   │      → 등록 URL 매칭
...
3. `application/stage_runner.build_serp_fetcher(settings.ranking_serp_fetcher)` 로 SERP HTML fetch — 기본 insane(curl-only, desktop+`#main_pack`) 우선 + Bright Data 폴백 하이브리드(무손실). insane 성공 시 `provider="insane"` cost=0 집계, selector 부정합/challenge 시 Bright Data 자동 폴백. `ranking_serp_fetcher` 는 분석 트랙 `crawler_serp_fetcher` 와 **독립 토글** — ranking cron 만 즉시 롤백(env `RANKING_SERP_FETCHER=brightdata`) 가능. `client.close()` 는 `FallbackFetcher.close` 가 insane+Bright Data 양쪽에 위임
...
- **재사용 자산**:
  ...
  - SERP fetcher(하이브리드): `application/stage_runner.build_serp_fetcher` (기본 insane #main_pack + Bright Data 폴백) + `domain/crawler/serp_collector.py`
...
| R4 | 매일 모든 publication SERP 체크 → Bright Data 비용 폭주 | ① 하이브리드 fetcher(`ranking_serp_fetcher` default insane) — insane #main_pack 우선 수집 시 cost=0, Bright Data 는 폴백만. **부하 실측**: ranking 실제 패턴(매 요청 close)으로 단일 IP 130건 무차단·100% strong_ok 확인 → 대량 cron 에서도 insane 기본 안전. ② publication 당 1초 sleep. ③ selector 부정합/차단 시 자동 Bright Data 폴백(무손실). 폴백률은 `provider=insane vs brightdata` usage 분포로 상시 관측, 급증 시 env 즉시 롤백 |
