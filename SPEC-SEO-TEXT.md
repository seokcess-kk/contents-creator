...
**수집 정책:**
- `ssc=tab.blog.all` 쿼리로 네이버 **블로그 전용 탭** 을 직접 요청 (2026-04-16 실측, lessons.md C2 참조).
  `where=blog` 통합검색 하위 섹션은 React 버튼으로 6~7개만 초기 렌더되고 `start` 파라미터도 무시되어 사용 불가.
...
POST https://api.brightdata.com/request
{
  ...
  "url": "https://search.naver.com/search.naver?ssc=tab.blog.all&query={keyword}&start=1",
  "format": "raw"
}
```
