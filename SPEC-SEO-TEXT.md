...
**수집 정책 — 통합검색 우선 + 블로그 탭 보충** (2026-04-16 실측 확정, lessons.md C2 참조):

제품 의도는 **"통합검색 상위 노출 블로그"** 분석이다. 따라서 항상 `where=blog` 통합검색부터 시도하고, 네이버가 통합검색 블로그 섹션에 6~7개만 초기 렌더하는 특성상 표본이 부족할 때만 `ssc=tab.blog.all` 블로그 전용 탭에서 보충한다. 각 결과에는 `source` 필드로 출처 트랙을 기록해 패턴 카드까지 전파한다.

1. **Step 1 — 통합검색**: `search.naver.com/search.naver?query={keyword}&where=blog` fetch. 있는 만큼 수집 (보통 6~7개)
2. **Step 2 — 블로그 탭 보충**: 통합검색 합계가 `MAX_RESULTS(10)` 미만이면 `search.naver.com/search.naver?ssc=tab.blog.all&query={keyword}&start=1` fetch. 중복 URL 제외하고 **최대 `BLOG_TAB_BOOST_LIMIT=5개`** 까지 이어 붙임
3. **Step 3 — 검증**: 합계가 `MIN_COLLECTED_PAGES(5)` 미만이면 `InsufficientCollectionError`

**기타 규칙**:
- 포스트 URL 은 `a[href]` 뿐 아니라 `*[data-url]` 속성에도 들어 있음 (네이버 신버전 UI). 파서는 둘 다 순회
- `blog.naver.com` / `m.blog.naver.com` 외 배제, `/clip/` (동영상) 배제, 광고 섹션 배제
- 통합검색 결과의 원 순위는 rank 1~N 에 보존되고 블로그 탭 보충분이 뒤에 이어짐
- 각 `SerpResult.source` 는 `"integrated"` | `"blog_tab"`
...
- **의미적 문단 단위는 `div.se-component.se-text` 하나** — 내부 `<p>` 는 네이버가 줄바꿈·정렬 단위로 10~15개씩 쪼개므로 p 단위로 세면 `total_paragraphs` 가 수백 개가 됨. `se-text` 컴포넌트 전체 텍스트를 공백 하나로 join 해 1개 paragraph 로 처리
- 구분선·인용구·표가 표준 HTML 태그가 아닌 네이버 자체 클래스 사용:
  - `<hr>` 대신 `div.se-horizontalLine`
  - `<blockquote>` 대신 `div.se-quotation`
  - `<table>` 대신 `div.se-table`
  → DIA+ 감지는 **표준 태그 + `se-*` 클래스 OR** 방식
- **소제목(heading) 감지는 상대 기준**: `<h1>`~`<h4>` 가 본문에 없고 `se-fs-fs{N}` 폰트 크기 클래스로만 구분됨. 크기 범위가 좁고(11~16) 블로그마다 기본 크기가 다르므로 **문서 전체 최빈 fs > body 이면서 길이 ≤ 80자** 일 때만 heading 판정
- **네이버 블로거 대부분이 실제로 소제목을 쓰지 않음**: 상위 10개 중 8개가 소제목 0개. [4a] 의미 추출은 소제목 0개 블로그를 "전체가 하나의 본문 섹션" 으로 폴백 처리해야 함
...
| 항목 | 기준 |
|---|---|
| SERP 수집 | 네이버 블로그 URL 최소 5개 수집 성공 |
| 본문 수집 | 재시도 후 최소 5개 HTML 파싱 가능 |
...
- `2026-05-06`: UX Refactor 6 Phase + Polish Pack 4 Phase 완료. **단일 흐름 시그니처 무변경** (정적 검증). 본 SPEC [1]~[10] 파이프라인은 그대로. frontend 만 변경: 신규 라우트 `/create` (단일 + CSV 배치 통합), `/queue` (검수·발행 큐 통합), nav 9→6 영역, redirect 어댑터 5개. 라우트 단일 출처: `docs/ROUTES.md`. Polish P4 가 `domain/generation/title_validator.py` 에 형태소 매칭 (kiwipiepy) helper 추가 — `validate_title()` 공개 시그니처 무변경, private helper 만
