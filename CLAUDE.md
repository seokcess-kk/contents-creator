...
### 🔴 분석 데이터 → 생성 반영 규칙 (Analysis-to-Generation Integrity)

패턴 카드의 분석 데이터가 최종 원고에 충실히 반영되어야 한다. **프롬프트만으로는 LLM 출력을 통제할 수 없다. 코드 검증 + 부분 재생성으로 강제한다.**
...
| Layer 3 | [10] 조립 시 | `assembler.py` | 이미지 위치를 LLM position 무시, 코드가 균등 배분 |

**데이터 전달 원칙 (`prompt_builder.py` 단일 경로):**
- 분석값이 0이거나 빈 배열일 때 그대로 전달 금지. 폴백 로직 필수 (예: 태그 0개 → 키워드 기반 8개 추천)
- 프롬프트 내 지시가 서로 모순되면 안 됨 (예: "소제목 1개" vs "필수 섹션 3개" 동시 지시 금지)
- 분석 수치 반올림 정밀도 주의 (키워드 밀도는 `round(x, 4)`)
- 소제목 수: 분석 avg < 2이면 `필수+빈출 섹션 수 기반 보정` (SEO를 위해 최소 4개)

**필수 반영 항목 — 프롬프트에 반드시 포함:**
- 글자수 범위 (`stats.chars.min`~`max`)
- 소제목 수 (분석값 또는 섹션 수 기반 보정값)
- 키워드 밀도 목표 + 배치 전략 (제목 앞, 첫문단, 마지막문단)
- DIA+ 요소 지시 (bold, blockquote 등 사용률 > 30% 항목)
- 이미지 개수 (분석 기반, `max(3, round(avg))`) + 위치 분포
- 톤앤매너: 친근한 전문가 톤 (1인칭 `저는/제가` 금지, 구어체 혼합)
- 태그 (분석 데이터 또는 키워드 기반 폴백 8개)
- 연관 키워드 (physical_extractor에서 공백 토큰화 + 조사 제거로 추출)
- 소구 포인트 (빈 배열이면 LLM 자체 구성 폴백)

**이미지 품질 규칙:**
- 이미지 배치: 코드가 `_build_even_image_map()`으로 균등 배분 (LLM position 무시)
- 이미지 삽입 무결성: outline 수 = 생성 수 = MD 수 = HTML `<img>` 수
- 이미지 크기: 생성 후 `max_width=720px` 리사이즈 + JPEG 변환 (settings.py)
- 이미지 종횡비: Gemini `ImageConfig(aspect_ratio=...)` API 레벨 제어
- 이미지 프롬프트: Gemini 공식 가이드 기반 장면 묘사형 (키워드 나열 금지)
- 이미지 소재: 인물/음식/공간/재료 자유 생성. 금지는 의료법 4항목만 (환자/시술/전후비교/신체노출)
- 인물: 무조건 한국인 (`Korean` 키워드 필수, 예외 없음)
- alt 텍스트: 주 키워드를 자연스럽게 포함 (SEO 최적화)
- 정형화 방지: 매 실행마다 분위기 시드 랜덤 선택

**검증 — 생성 후 자동 체크:**
- `quality-report.json`: 글자수 ±20%, 키워드 밀도, 이미지 일치
- `outline_validator.py`: 섹션 수 ≥ 필수+빈출, 이미지 수 ≥ 목표
- `body_quality_enforcer.py`: 섹션별 글자수 ≥ 목표의 80%, 키워드 포함 여부
- `tests/fixtures/naver_html/`: 실측 HTML fixture로 물리 분석 regression 테스트 (12개)
...
- checker에 SEO 키워드 컨텍스트 전달 — 키워드 자체를 `first_person_promotion`으로 오탐하지 않도록
- fixer에 금지 표현 목록 주입 — 수정 시 새 위반 생성 방지
- LLM이 violations에 dict 대신 str 반환하는 케이스 방어 (`_parse_llm_violations`)
- 톤 지시와 의료법 규칙 충돌 주의: "경험 공유"가 1인칭 체험기를 유도할 수 있음 → "친근한 전문가 톤" 사용
...
- `2026-04-16`: SEO 파이프라인 전면 개선. 3계층 품질 강제 시스템 (outline 검증 + body 부분 보강 + 이미지 코드 배치). 물리 분석 정확도 (키워드 공백 매칭, 문단 p태그 단위, 소제목 감지). Gemini 공식 가이드 반영 (ImageConfig aspect_ratio). 의료법 checker/fixer 안정화. 톤앤매너 대화체 전환. Supabase 저장. [4a]/[4b] 병렬화. regression 테스트. lessons.md SEO-1~4 교훈
