...
네이버 키워드 → Bright Data 크롤링 → 상위글 분석 → 패턴 카드 → SEO 원고 → 의료법 검증 → 네이버 호환 출력의 8단계 내부 파이프라인 도구.
...
- Phase 2 FastAPI 도 `application.orchestrator.*` 를 호출 (SPEC.md §12 참조)
...
- @SPEC.md — 전체 기획서 (8단계 파이프라인, 스키마, 모델 매핑, 검증 기준)
...
- @SPEC.md §12 — Phase 2 Web UI 대비 (Next.js + FastAPI 확장 지점)
