...
패턴 카드의 분석 데이터가 최종 원고에 충실히 반영되어야 한다. 프롬프트 빌더(`prompt_builder.py`)가 유일한 전달 경로이며, 다음 원칙을 준수한다:

**데이터 전달 원칙:**
- 분석값이 0이거나 빈 배열일 때 그대로 전달하지 말 것. 폴백 로직 필수 (예: 태그 0개 → 키워드 기반 8개 추천)
- 프롬프트 내 지시가 서로 모순되면 안 됨 (예: "소제목 1개" vs "필수 섹션 3개" 동시 지시 금지)
- 분석 수치를 프롬프트에 넣을 때 반올림 정밀도 주의 (키워드 밀도 0.002를 round(x,2)로 0.00이 되는 사례 발생함)

**필수 반영 항목 — 프롬프트에 반드시 포함:**
- 글자수 범위 (`stats.chars.min`~`max`)
- 소제목 수 (분석값 또는 섹션 수 기반 보정값)
- 키워드 밀도 목표 + 배치 전략 (제목, 첫문단, 마지막문단)
- DIA+ 요소 지시 (bold, blockquote 등 사용률 > 30% 항목)
- 이미지 개수 + 위치 분포 (분석 기반)
- 톤앤매너 (도입 유형 분포 기반)
- 태그 (분석 데이터 또는 키워드 기반 폴백)

**이미지 삽입 무결성:**
- outline의 `image_prompts` 수 = 생성된 이미지 수 = MD 내 이미지 수 = HTML 내 `<img>` 수
- 섹션 번호가 heading 수를 초과해도 문서 끝에 삽입 (누락 0건 보장)
- `quality-report.json`에서 이미지 불일치 시 경고

**검증 — 생성 후 자동 체크 (`quality-report.json`):**
- 글자수: 분석 범위 ±20% 이내
- 키워드 밀도: 분석 목표 50~200% 이내
- 이미지: outline 수 = md 수 = html 수 일치
