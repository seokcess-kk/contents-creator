...
- `2026-05-06`: UX Refactor Phase 1~4 진행 중 — Operations OS 정렬. P1 nav 9→6 영역, `/` 운영 홈 승격, `/rankings` `/pipeline` 영구 redirect. P2 ui primitives 8종 (Button/Dialog/StatusBadge/MetricStrip/DataTableShell/EmptyState/PageHeader/ActionBar) + lucide-react. P3 PublicationActionRow 384→196줄 정보밀도 정리, primary CTA + ⋯ Dropdown, lib/labels.ts (P6 선행). P4 `/create` 통합 페이지 (단일 키워드 / CSV 배치 탭), `/legacy-jobs` 제거, `/batches` BatchUploadForm 분리. 단일 흐름 시그니처 무변경. SPEC: `tasks/todo.md` "UX Refactor — Operations OS 정렬"
