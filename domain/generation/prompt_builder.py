...
def _build_outline_system(
    ...
    return (
        ...
        "- '~입니다' 일변도 금지. "
        "'~예요/~이에요', '~거든요', '~인데요' 등 부드러운 문체를 섞어라\n"
        "- 독자가 공감할 수 있는 상황 묘사 포함 "
        "(예: '이런 경험 있으시죠?', '많은 분들이 고민하는 부분인데요')\n"
        "- 🔴 1인칭 금지: '저는', '제가', '저도', '저의' 등 "
        "화자 본인의 체험기 형식 절대 금지. "
        ...
        f"5. 키워드 '{pc.keyword}'를 주체로 쓰지 마라. "
        f"'{pc.keyword}에서는', '{pc.keyword}을 찾는 환자' 등 "
        "특정 기관을 주어로 내세우는 서술 금지. "
        "키워드는 정보 검색의 맥락으로만 사용\n"
        "6. 리스트를 중첩하지 말 것. "
...
def _build_body_system(
    ...
    return (
        ...
        "2번째 섹션부터 작성하며, 친근한 전문가 톤으로 서술한다.\n"
        "딱딱한 정보 나열 금지. 부드러운 구어체를 섞되, "
        "1인칭(저는/제가/저도) 체험기 형식은 절대 금지.\n\n"
        f"[톤 힌트]\n{intro_tone_hint}\n"
...
def _format_image_instructions(pc: PatternCard) -> str:
    ...
    if avg > 0:
        target = max(3, min(round(avg), 10))  # 하한 3, LLM 부담 방지 상한 10
