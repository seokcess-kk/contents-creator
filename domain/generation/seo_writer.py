...
이 원고는 브랜디드 카드 이미지와 함께 하나의 리치 블로그 글로 조합됩니다.
...
## 카드 삽입 마커 (필수)
본문 중 아래 마커를 반드시 적절한 위치에 삽입하세요. 마커는 별도 줄에 단독으로 작성합니다.
- `<!-- CARD:intro -->` — 도입부 직후 (업체 브랜드 소개 카드 이미지가 삽입됩니다)
- `<!-- CARD:transition -->` — 고민에서 솔루션으로 전환되는 지점
- `<!-- CARD:cta -->` — 글 마지막, CTA 카드 이미지가 삽입됩니다
