...
1. **모든 Bright Data 호출은 `brightdata_client.py` 경유** — 개별 파일에서 `requests.post()` 직접 호출 금지. **본문([2])·SERP([1])·난이도(keyword_difficulty)·ranking(cron) 수집은 `HtmlFetcher` 추상화(`fetcher.py`) 경유** — 기본 insane(curl-only, cost=0) 우선 + Bright Data 폴백(`fallback_fetcher.py`). 본문은 mobile default, SERP·난이도·ranking 은 desktop + `success_selectors=["#main_pack"]` 튜닝(insane #main_pack 우선, selector 부정합/challenge 시 Bright Data 폴백 — 무손실). 라우팅 토글: 본문 `crawler_body_fetcher`, SERP·난이도 `crawler_serp_fetcher`, ranking(cron) `ranking_serp_fetcher`(모두 application 팩토리 `build_serp_fetcher`/`_build_body_fetcher`). **이제 본문·SERP·난이도·ranking 전부 하이브리드**(ranking 은 `ranking_serp_fetcher` default insane #main_pack + Bright Data 폴백, 분석 트랙과 독립 롤백 토글)
...
- `serp_collector.py` — SERP 쿼리 + 네이버 블로그 URL 필터 + 선착순 10개 선택. `HtmlFetcher` 를 받아 fetch (라우팅은 application `build_serp_fetcher(fetcher_choice)` — 기본 insane #main_pack + Bright Data 폴백. 분석 트랙/ranking 이 각자 토글 값을 인자로 주입). `build_main_search_url` 통합검색 SERP 는 ranking cron 도 동일 팩토리 경유
...
- 임의 대체 크롤링 라이브러리 추가 금지 — 본문·SERP·난이도·ranking 수집은 **승인된 `vendor/insane_search`(curl-only, v0.9.1) 우선 + Bright Data 폴백** 하이브리드. 이 외의 라이브러리(Scrapling, Playwright 등, insane 의 Playwright Phase 포함) 도입은 여전히 금지
