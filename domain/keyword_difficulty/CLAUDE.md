...
# 보고용 점수 (낮을수록 노출 유리)
score = D × 1.5 - B × 3 + keyword_adjustment
...
# 키워드 보정
# - 전국 대표/보장 제외급 의료 키워드: score +12, 최소 HIGH
# - 병원/한의원/처방/가격/후기 등 고전환·검수 리스크 키워드: score +6, 최소 MEDIUM
```

임계값 (`8`/`2`/`5`/`0.5`) 은 운영 데이터 누적 후 조정 가능. 변경 시 본 CLAUDE.md
의 공식 + `scorer.py` 의 상수/키워드 보정 목록 + 회귀 테스트의 기댓값을 동시에 갱신할 것.
